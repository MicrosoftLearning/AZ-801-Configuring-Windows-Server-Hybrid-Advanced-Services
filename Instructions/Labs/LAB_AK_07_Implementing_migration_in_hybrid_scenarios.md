...
1. On the **Custom deployment** page, specify the following settings (leave others with their default values):
...
1. On the **Custom deployment** page, select **Review + create**, and then select **Create**.
...
1. From the **az801l07a-hv-vm** page, connect to the operating system running within the virtual machine via Azure Bastion by using the following credentials:
...
1. On the **Azure Migrate | az801l07a-migrate-project** page, under **Execute** on the left menu, select **Migrations**, and then select **Start execution**.
...
1. On the provider setup page, specify the following settings, and then select **Create resources**:
   ...
   | Confirm that the target region for migration is | selected |
...
1. In **Prepare Hyper-V host servers**, select the **Download the Hyper-V replication provider** link to download the `AzureSiteRecoveryProvider.exe` software installer.
   ...
   ![The provider setup page with arrows pointing to the replication provider link and registration key Download button.](media/lab07-download-provider-setup.png)
...
1. When installation completes, select **Register**. This starts the **Microsoft Azure Site Recovery Registration Wizard**.
...
1. On the final **Registration** page of the **Microsoft Azure Site Recovery Registration Wizard**, select **Finish**.
1. Return to the provider setup page in the Azure portal, and then select **Finalize registration**.
1. Wait for confirmation that registration is finalized.
...
1. On the **Target settings** page, specify the following settings (leave others with their default values), and then select **Next**:
...
1. On the **az801l07a-vm1** page, select **Clean up test migration** from the menu bar.
1. On the **Test migrate cleanup** page, select **Testing is complete. Delete test virtual machine**, and then select **Cleanup Test**.
...
1. From the **Cloud Shell** pane, run the following command to display the resource locks created during the migration:

   ```powershell
   Get-AzResourceLock -ResourceGroupName 'AZ801-L0703-RG'
   ```

1. From the **Cloud Shell** pane, run the following command to remove the locks created by Azure Site Recovery:
